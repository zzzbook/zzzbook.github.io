...
- **理解过程**：
  ...
  $\begin{array}{l} & min f_0(x),x\in\mathbb{R} \newline & s.t. \begin{cases} f_i(x) \le 0,其中i=1,2,3,...,m \newline h_i(x) = 0,其中i=1,2,3,...,q \end{cases} \newline \Rightarrow & \{(y,u)\vert t=f_{0}(x),u_i=f_i(x),x\in D\}=G_1 \newline & D = \left\{ x \bigg\vert \enspace \begin{array}{l} f_i(x)\le0,其中i=1,2,3,...,m \newline h_i(x) = 0,其中i=1,2,3,...,q \end{array} \right\} \newline \Rightarrow & min_x \{t\vert (t,u)\in G_1,u\le0\} \end{array}$ 
